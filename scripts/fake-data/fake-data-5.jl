...
# BUT WITH REFINEMENTS OF: THE EXPERIMENT IS DONE IN IMMUNOCOMPETENT MICE & NEUTROPHILS ARE RECORDED & ALL SPECIES ARE RECORDED AT 12, 32 AND 72 HRS INSTEAD OF 24, 48 & 72 HRS.
...
n_sims = 10  #number of simulated reps of the simulation study
...
t_pts = [12.0, 32.0, 72.0]  #time points of planned experimental data
tspan = (0.0, t_pts[end])
...
function quant_rev(real_data::DataFrame,
    ...
    if condition == "High IC"   #different experimental conditions have different ICs
        p = p_draw[2:end]
    else
...
new_exp_var = vcat(vars[2], vars[1][2])  #stores the vars of F_all (low IC), C and N (high IC)
new_exp_max = vcat(maxs[2], maxs[1][2])
cd("/$path/fake-data")
try
    mkdir("fake-data-5")   #if it doesnt exist make the folder
    cd("fake-data-5")
catch err
    cd("fake-data-5")
end
for idx in 1:no_of_param_set
    ...
    for (idy, noise) in enumerate(noises)
        ...
        for idz in 1:n_sims
            ...
            for (idw, elt) in enumerate(unique(real_data.Condition))  #begin with making fake data sets of the previously collected in vivo data
            ...
            starting_exp_no = maximum(vcat([parse.(Int, (x -> x[2]).(split.(unique(dfs[i].Experiment), "E"))) for i in 1:2]...))  #want the exps to have unique exp numbers
