...
#  HERE FAKE DATA IS DATA THAT MIMICS THE DATA SPARSITY OF A CONVENTIONALLY DESIGNED EXPERIMENT (fake-data-2.jl)
...
for i in collect(1:n_sims)
    all_data[i] = CSV.read("/$path/fake-data/fake-data-2/parameter-set-$p_set/noise-$chosen_noise/fake-data-set-$i.csv", DataFrame)
end
@everywhere begin
    #
    # ODE arguments
...
for i in 1:n_sims
    working_result[:, i] = readdlm("/$path/data/fitted-data/fake-data/fitted-data-2/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$i.csv", ';', Float64)
end
