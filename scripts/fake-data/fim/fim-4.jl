...
#  CONVENTIONALLY DESIGNED EXPERIMENT BUT REFINED SO THAT IT WAS CONDUCTED WITH IMMUNOCOMPETENT MICE AND NEUTROPHILS ARE RECORDED (fake-data-4.jl)
...
for i in collect(1:n_sims)
    all_data[i] = CSV.read("/$path/fake-data/fake-data-4/parameter-set-$p_set/noise-$chosen_noise/fake-data-set-$i.csv", DataFrame)
end
@everywhere begin
    #
    # ODE arguments
...
for i in 1:n_sims
    working_result[:, i] = readdlm("/$path/data/fitted-data/fake-data/fitted-data-4/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$i.csv", ';', Float64)
end
