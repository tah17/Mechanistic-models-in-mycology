...
#  HERE FAKE DATA IS DATA THAT MIMICS THE DATA SPARSITY OF THE PREVIOUSLY COLLECTED DATA IN MYCOLOGY AND A
#  CONVENTIONALLY DESIGNED EXPERIMENT BUT REFINED SO THAT IT WAS CONDUCTED WITH IMMUNOCOMPETENT MICE, NEUTROPHILS ARE RECORDED AND SPECIES ARE
#  RECORDED AT 12, 32 AND 72 HOURS INSTEAD  (fake-data-5.jl)
...
@everywhere begin
    ...
    global bounded = true  #arguments to change
    n_sims = 100
    path = joinpath(@__DIR__, "../../..")
end
@everywhere Random.seed!(3744613754752089)  #julia RNGs
...
for i in collect(1:n_sims)
    all_data[i] = CSV.read("/$path/fake-data/fake-data-5/parameter-set-$p_set/noise-$chosen_noise/fake-data-set-$i.csv", DataFrame)
end
@everywhere begin
    #
    # ODE arguments
...
for i in 1:n_sims
    working_result[:, i] = readdlm("/$path/data/fitted-data/fake-data/fitted-data-5/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$i.csv", ';', Float64)
end
