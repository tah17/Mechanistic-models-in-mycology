...
#  AND A PROPOSED CONVENTIONAL EXPERIMENTAL DESIGN BUT THE EXPERIMENT IS DONE IN IMMUNOCOMPETENT MICE, NEUTROPHILS ARE RECORDED AND ALL SPECIES RECORDED AT 12, 32 AND 72 HOURS (fake-data-5.jl)
...
batch_no = #get batch no
batch_rg = mod(batch_no, n_sims)
if batch_rg == 0
    batch_rg = n_sims
end
...
#
# read in arguments passed to command line - details what parameter set to read in (used to generate the fake data in fake-data-5.jl)
#
...
#
# records job IDs - NOTE need file bootstrap-5-ids.txt already created and in same directory as submission file
#
job_id = #get job id
job_recorded = false
open("bootstrap-5-ids.txt") do io
...
open("bootstrap-5-ids.txt", "a") do io
  if !job_recorded
      write(io, "Algo $algo with parameter $p_set: $job_id\n")
  end
end;
#
# read in the fake data set that was used for fitting (created by fake-data-5.jl) specific to this batch no.
#
...
#
# read in MLEs from fitting to the fake data (generated by running fit-data-5.jl) specific to this batch no.
#
working_result = readdlm("/$path/data/fitted-data/fake-data/fitted-data-5/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$(batch_rg).csv", ';', Float64)
