...
@everywhere begin
    ...
    n_sims = 100
    b_reps = 100   #number of bootstrap replicates
...
#
# records job IDs - NOTE need file bootstrap-1-ids.txt already created and in same directory as submission file
#
job_id = # get job id
job_recorded = false
open("bootstrap-1-ids.txt") do io
...
open("bootstrap-1-ids.txt", "a") do io
  if !job_recorded
      write(io, "Algo $algo with parameter $p_set: $job_id\n")
  end
end;
#
# read in the real data set to specify ODE arguments
#
...
@everywhere function user_defined(real_data::DataFrame,   #all workers require access to this fn
                                  ...
                                  Example_ODE::Function, p_draw::AbstractArray{Float64,1}, solver::OrdinaryDiffEq.OrdinaryDiffEqAlgorithm, saveat::Real;
...
#
# read in the fake data that was used for fitting (created by fake-data-1.jl) specific to this batch no.
#
...
if algo == "BH"
    ...
    @sync @distributed for i in 1:b_reps  #perform the bootstrap fitting - needs @sync to avoid lack of reproducibility that happens with dynamic dispatch
        ...
        data_b = bootstrap_sample(fake_data, julia_rngs[myid() - 1])   #sample a bootstap data set by bootstrapping by experiments, and use julia seed labelled by worker ID (which begin at 2 and end at 7)
