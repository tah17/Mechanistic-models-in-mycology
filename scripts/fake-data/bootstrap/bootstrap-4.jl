...
#  HERE FAKE DATA IS DATA THAT MIMICS THE DATA SPARSITY OF THE PREVIOUSLY COLLECTED DATA IN MYCOLOGY
#  AND A PROPOSED CONVENTIONAL EXPERIMENTAL DESIGN BUT THE EXPERIMENT IS DONE IN IMMUNOCOMPETENT MICE  AND NEUTROPHILS ARE RECORDED (fake-data-4.jl)
...
tot_rg = Array{Any,1}(undef, n_sims*nworkers())  #again, need a new stream for each simulated replicate (n_sims) but also for each worker with access to the script (nworkers())
for i in 1:n_sims*nworkers()
...
#
# read in arguments passed to command line - details what parameter set to read in (used to generate the fake data in fake-data-4.jl)
#
...
#
# records job IDs - NOTE need file bootstrap-4-ids.txt already created and in same directory as submission file
#
job_id = #get job id
job_recorded = false
open("bootstrap-4-ids.txt") do io
...
open("bootstrap-4-ids.txt", "a") do io
  if !job_recorded
      write(io, "Algo $algo with parameter $p_set: $job_id\n")
  end
end;
#
# read in the fake data set that was used for fitting (created by fake-data-4.jl) specific to this batch no.
#
fake_data = CSV.read("/$path/fake-data/fake-data-4/parameter-set-$p_set/noise-$chosen_noise/fake-data-set-$batch_rg.csv", DataFrame)
@everywhere begin
    #
    # ODE arguments
    #
...
#
@everywhere function user_defined(real_data::DataFrame,
                                  condition::String,
...
#
# read in MLEs from fitting to the fake data (generated by running fit-data-4.jl) specific to this batch no.
#
working_result = readdlm("/$path/data/fitted-data/fitted-data-4/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$(batch_rg).csv", ';', Float64)
...
if algo == "BH"
    @everywhere begin
        ...
        p0s_tot = [vcat( (10 .^ vcat(rand($RNG, Normal(4, 1)), rand($RNG, Normal(7, 1)), rand($RNG, Normal(0, 1), no_of_species - 1)))./10^6, rand($RNG, Truncated(Normal(0, 1), 0, 4), no_of_params - no_of_species - 1)) for i in 1:n_sims]
    ...
    @sync @distributed for i in 1:b_reps  #perform the bootstrap fitting - needs @sync to avoid lack of reproducibility that happens with dynamic dispatch
        p0_i = vcat( (10 .^ vcat(rand(julia_rngs[myid() - 1], Normal(4, 1)), rand(julia_rngs[myid() - 1], Normal(7, 1)), rand(julia_rngs[myid() - 1], Normal(0, 1), no_of_species - 1)))./10^6, rand(julia_rngs[myid() - 1], Truncated(Normal(0, 1), 0, 4), no_of_params - no_of_species - 1))   #starting guess for algo that depends on the a specified stream
...
elseif algo == "DE"
    @everywhere begin
        ...
        p_range = vcat(tuple(0, 1), tuple(0, 1000), [tuple(0, 4) for i in 1:(no_of_params - 2)])  #parameter ranges needed for DE algo
