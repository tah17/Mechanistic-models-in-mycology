...
#  AND A PROPOSED CONVENTIONAL EXPERIMENTAL DESIGN BUT THE EXPERIMENT IS DONE IN IMMUNOCOMPETENT MICE (fake-data-3.jl)
...
#
# read in arguments passed to command line - details what parameter set to read in (used to generate the fake data in fake-data-3.jl)
#
...
#
# records job IDs - NOTE need file bootstrap-3-ids.txt already created and in same directory as submission file file
#
job_id = #get job id
job_recorded = false
open("bootstrap-3-ids.txt") do io
...
open("bootstrap-3-ids.txt", "a") do io
  if !job_recorded
      write(io, "Algo $algo with parameter $p_set: $job_id\n")
  end
end;
#
# read in the fake data set that was used for fitting (created by fake-data-3.jl) specific to this batch no.
#
fake_data = CSV.read("/$path/fake-data/fake-data-3/parameter-set-$p_set/noise-$chosen_noise/fake-data-set-$batch_rg.csv", DataFrame)
@everywhere begin
    #
    # ODE arguments
    #
...
#
# read in MLEs from fitting to the fake data (generated by running fit-data-3.jl) specific to this batch no.
#
working_result = readdlm("/$path/data/fitted-data/fake-data/fitted-data-3/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$(batch_rg).csv", ';', Float64)
