...
# BUT WITH THE REFINEMENT OF: THE EXPERIMENT IS DONE IN IMMUNOCOMPETENT MICE
...
# parameters are in formact [F_0_low, F_0_high, C_0, N_0, F_D_0, p_1, ...., p_9]
#
...
vars = []
maxs = []
for (idx, elt) in enumerate(unique(real_data.Condition))   #gets the variance and maximums of the currently observed data (split by F_all, C & N)
...
new_exp_var = vcat(vars[2], vars[1][2])  #stores only the vars of F_all and C
...
for idx in 1:no_of_param_set   #iterates through true parameters
    try
    ...
    for (idy, noise) in enumerate(noises)   #iterates through noise
        try
            mkdir("noise-$idy")
            cd("noise-$idy")
        catch err
        ...
        for idz in 1:n_sims
            ...
            starting_exp_no = maximum(vcat([parse.(Int, (x -> x[2]).(split.(unique(dfs[i].Experiment), "E"))) for i in 1:2]...))  #ensure unique exp nunbers in dataframes
