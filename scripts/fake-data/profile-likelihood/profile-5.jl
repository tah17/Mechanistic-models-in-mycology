...
#
# records job IDs - NOTE need file profile-5-ids.txt already created and in same directory as submission file
#
job_id = #job id
job_recorded = false
open("profile-5-ids.txt") do io
...
open("profile-5-ids.txt", "a") do io
  if !job_recorded
...
cd("/$path/data")
try
    mkdir("profile-intervals-5")
    cd("profile-intervals-5")
catch err
    cd("profile-intervals-5")
end
try
