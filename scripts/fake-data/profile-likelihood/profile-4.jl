...
#
# records job IDs - NOTE need file profile-4-ids.txt already created and in same directory as submission file
#
job_id = #job id
job_recorded = false
open("profile-4-ids.txt") do io
...
open("profile-4-ids.txt", "a") do io
  if !job_recorded
...
#
working_result = readdlm("/$path/data/fitted-data/fake-data/fitted-data-4/$algo/parameter-set-$p_set/noise-$chosen_noise/working-result-$(batch_rg).csv", ';', Float64)
...
cd("/$path/data")
try
    mkdir("profile-intervals-4")
    cd("profile-intervals-4")
catch err
    cd("profile-intervals-4")
end
try
