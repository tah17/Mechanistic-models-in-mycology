...
#
# records job IDs - NOTE need file profile-2-ids.txt already created and in same directory as submission file
#
job_id = #get job_id
job_recorded = false
open("profile-2-ids.txt") do io
...
open("profile-2-ids.txt", "a") do io
  if !job_recorded
...
#
scan_bounds = vcat(tuple(1e-9, 1.0 - 1e-9), [tuple(1e-9, 4.0 - 1e-9) for i in 1:(no_of_params - 1)])
theta_bounds = vcat(tuple(0.0, 1.0), [tuple(0.0, 4.0) for i in 1:(no_of_params - 1)])
...
cd("/$path/data")
try
    mkdir("profile-intervals-2")
    cd("profile-intervals-2")
catch err
    cd("profile-intervals-2")
end
try
