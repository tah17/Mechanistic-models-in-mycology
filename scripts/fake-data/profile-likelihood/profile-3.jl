...
#
# records job IDs - NOTE need file profile-3-ids.txt already created and in same directory as submission file
#
job_id = #get job id
job_recorded = false
open("profile-3-ids.txt") do io
...
open("profile-3-ids.txt", "a") do io
  if !job_recorded
...
cd("/$path/data")
try
    mkdir("profile-intervals-3")
    cd("profile-intervals-3")
catch err
    cd("profile-intervals-3")
end
try
