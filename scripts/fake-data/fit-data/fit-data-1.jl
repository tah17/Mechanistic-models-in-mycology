#
#  BATCH JOB SCRIPT THAT FITS THE MODEL TO ITS OWN FAKE DATA - HERE FAKE DATA IS DATA THAT MIMICS THE DATA SPARSITY OF THE PREVIOUSLY COLLECTED DATA IN MYCOLOGY (fake-data-1.jl)
...
#
# set python RNGs (for SciPy algos)
#
...
for i in 1:n_sims  #generate n_sims streams
    tot_rg[i] = npr.Generator(bit_generator)
    global bit_generator = bit_generator.jumped()
end
batch_no = #get batch no, if there are 5 noises there will be 500 batch numbers (5*n_sims)
batch_rg = mod(batch_no, n_sims)     #only need to have independent streams for the same noise i.e. for 100 batch jobs
if batch_rg == 0
    batch_rg = n_sims
end
rg = tot_rg[batch_rg]   #pick the stream based off the batch number
include("../../../src/data-handling.jl")
include("../../../src/fit.jl")
@pyimport scipy.optimize as so
#
# read in arguments passed to command line
# details what parameter set to read in (used to generate the fake data in fake-data.jl) and
# what SciPy algorithm to use
#
algo = ARGS[1]
p_set = parse(Int64, ARGS[2])
chosen_noise = ceil(Int64, batch_no/n_sims)
#
# records job IDs - NOTE need file fit-1-ids.txt already created and in same directory as sub file
#
job = #get job id
job_recorded = false
open("fit-1-ids.txt") do io
...
open("fit-1-ids.txt", "a") do io
  if !job_recorded
      write(io, "Algo $algo with parameter $p_set: $job_id\n")
  end
end;
#
# read in the real data set
#
...
if algo == "BH"
    ...
    function wrap_pyfun(cost::Function, p0::Array{Float64,1}; niter::Union{Int64, Nothing}, seed::Any) #wrapper avoids an error encountered with some PyCall functions and @distributed macro
        return pyoptim.pyfun(cost, p0, niter = niter, seed = seed)  #... apart from basin hopping
    end
    #
    # fit the ODE
    #
